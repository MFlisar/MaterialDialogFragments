...
internal class DialogMonthDayFragment : MaterialDialogFragment<DialogMonthDay>(),
    ...
    override fun onHandleCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        dialog
            .negativeButton(setup) {
                sendEvent(DialogMonthDay.Event.Empty(setup, MaterialDialogButton.Negative))
                dismiss()
            }

            .neutralButton(setup) {
                sendEvent(DialogMonthDay.Event.Empty(setup, MaterialDialogButton.Neutral))
            }
    ...
    fun updateView(init: Boolean) {
        ...
        UIUtil.setAdapter(
            ...
            MonthDayType.values().map { requireActivity().getString(it.typeRes) }.toMutableList(),
        ...
        UIUtil.setAdapter(
            ...
            init,
            binding.spDayNumber,
            dayNumbers.map { requireActivity().getString(R.string.mdf_day_n, it + 1) }
                .toMutableList(),
            dayNumber,
            false
        )
        binding.spDayNumber.visibility =
        ...
        UIUtil.setAdapter(
            ...
            dayNumberBeginningValues.map { requireActivity().getString(it) }.toMutableList(),
        ...
        binding.spWeekday.visibility =
            if (monthDayType == MonthDayType.DayOfWeek) View.VISIBLE else View.GONE
        ...
        UIUtil.setAdapter(
            ...
            weekDayNumbers.map { requireActivity().getString(R.string.mdf_weekday_n, it + 1) }
                .toMutableList(),
            weekdayNumber,
            false
        )
        binding.spWeekdayNumber.visibility =
            if (monthDayType == MonthDayType.DayOfWeek) View.VISIBLE else View.GONE

        // 6) Info
        val monthDay = createMonthDay()
        binding.tvInfo.text = monthDay.toReadableString(requireActivity(), false)
    }

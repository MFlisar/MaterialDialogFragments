...
abstract class DialogFastAdapterFragmentOld : MaterialDialogFragment<DialogFastAdapterOld>() {

    companion object {

        fun <T : DialogFastAdapterFragmentOld> create(setup: DialogFastAdapterOld, createFragment: (() -> T)): T {
    ...
    protected open fun onHandleClick(item: IItem<*>?, buttonIndex: Int?, position: Int?) {
        val data = if (item != null && position != null) {
            DialogFastAdapterEvent.Data(position, item)
        } else null
